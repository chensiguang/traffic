 ...
 */
public class SafeLearnAction extends BaseAction
{
	...
	@Autowired
	private SafeLearnService safelearnService;
	...
	public String viewEduList()
	{
		...
		this.pageString = this.getPageNumView(curpage, pagenum,
				"page/admin/page/safelearnback_viewEduList.action");
	...
	{
		HttpSession session = ServletActionContext.getRequest().getSession();
		// long admin = (Long) session.getAttribute("adminflag");//
		// 获得当前登录用户是否为数据管理员的标记
		// long dwid = (Long) session.getAttribute("dwid");// 获得当前登录用户所在单位号
		long dwtype = (Long) session.getAttribute("dwtype");// 获得当前登录用户所在单位类型
		String driverid = ((String) session.getAttribute("driverid"));// 获得当前登录用户的id用户号

		String time = this.getParameter("time");// 时间以秒计算

		String videoid = this.getParameter("videoid");

		SafeLearn sl = new SafeLearn();

		sl.setDate(new Date());

		sl.setTime(Integer.parseInt(time));

		sl.setVideoid(Integer.parseInt(videoid));

		sl.setDriverid(Integer.parseInt(driverid));

		safelearnService.insertime(sl);

		if(dwtype == 2)
		{
			Driver driver = driverService.queryLearntime(Integer.parseInt(driverid));
			int learntime = 0;

			if (driver != null)
			{
				learntime = Integer.parseInt(driver.getLearntime())
			...
			query.put("learntime", learntime);

			driverService.updateLearntime(query);
			
			message = "视频观看时间保存成功！";
		}
		action = "page/admin/page/safelearnback_viewEduList.action";
