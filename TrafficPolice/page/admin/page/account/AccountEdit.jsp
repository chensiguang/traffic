		...
		<div class="pageTitle"  align="left">${title}</div>
			<table cellpadding="0" cellspacing="5" width="600px">
				<tr style="height: 45px">
				 ...
				 <tr style="height: 45px">
					...
					<s:iterator value="orgList" status="org">
						 <option value='<s:property value="id"/>' <s:if test="org.id = dom.dwno">selected="selected"</s:if>><s:property value="orgname"/></option>
					</s:iterator>
				...
				<tr style="height: 45px">
					<td>用户姓名:</td>
					<td>
					<select name="dom.dwno" id="dwno" style="width: 200px; height: 22px;">
				...
				<tr style="height: 45px">
					<td>密码:</td>
					<td>
					<input style="width: 200px; height: 22px;" type="text" name="dom.password" value="${dom.password}" required="true"/>&nbsp;<font color="#FF0000">*</font>
					</td>
			  </tr>
				...
				<tr style="height: 45px">
					...
					<s:iterator value="roleList" status="role">
						 <option value='<s:property value="roleno"/>' <s:if test="role.roleno = dom.roleno">selected="selected"</s:if>><s:property value="rolename"/></option>
					</s:iterator>
					</select>&nbsp;<font color="#FF0000">*</font>
					</td>
				</tr >
			...
			<tr style="height: 45px">
					<td>账号类型:</td>
					<td>
					<select name="dom.typeid">
						<option value="0">总账号</option>
	...
	$.ajax({
		...
		//alert(data);
			if(re.size > 0)
			{
				...
				{
					driver +='<div class=info id="driverinfo" onclick="adddriver()">'+re.drivers[i].name+'</div>';
				}
			}
