		...
		<div class="pageTitle"  align="left">${title}</div>
			<table cellpadding="0" cellspacing="5" width="600px">
				<tr style="height: 45px">
					...
					<input name="dom.typeid" value="0" type="hidden">	
						</td>
				</tr>
				...
				<tr style="height: 45px">
					<td>所属单位:</td>
					<td>
					<select name="dom.dwno" id="dwno" style="width: 200px; height: 22px;" required="true" onchange="selectpolicename()">
						<option value="">请选择单位...</option>
					<s:iterator value="orgList" status="org">
						 <option value='<s:property value="id"/>' <s:if test='dom.dwno == id'>selected="selected"</s:if>><s:property value="orgname"/></option>
					</s:iterator>
				...
				<tr style="height: 45px">
					...
					<td>
					<select name="dom.username"  style="width: 200px; height: 22px;"  id="policeshtml">
				...
				<tr style="height: 45px">
					<td>角色:</td>
					<td>
					<select name="dom.roleno" style="width: 200px; height: 22px;" required="true">
...
$(function(){
	$("#accountform").html5Validate(function(){
		this.submit();
				},{
				});
});

$(document).ready(function selectpolicename()
{
	var dwid = $("#dwno").val();
		var police ='<option value="">请选择用户姓名...</option>';
	$.ajax({
		type:'post',//可选get
		url:'/TrafficPolice/page/admin/page/account_policelist.action',//这里是接收数据的PHP程序
		...
		var re =eval('('+data+')');
			if(re.size > 0)
			{
				for(var i =0 ; i < re.polices.length;i++)
				{
					police +='<option <s:if test="dom.username == '+re.polices[i].id+'">selected="selected"</s:if> value='+re.polices[i].id+'>'+re.polices[i].name+'</option>';
				}
			}
			$('#policeshtml').html(polices);
	...
	$.ajax({
		...
		//alert(re);
			if(re.size > 0)
			{
				for(var i =0 ; i<re.polices.length;i++)
				{
					police +='<option <s:if test="dom.userno == '+re.polices[i].id+'">selected="selected"</s:if> value='+re.polices[i].id+'>'+re.polices[i].name+'</option>';
				}
			}
			$('#policeshtml').html(police);
